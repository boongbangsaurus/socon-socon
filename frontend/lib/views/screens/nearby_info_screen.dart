...
class _NearbyInfoScreenState extends State<NearbyInfoScreen> {
  ...
  Future<List<Store>?> _fetchStores() async {
    try {
      await Future.delayed(Duration(seconds: 2)); // 이 부분은 테스트 목적이라면 그대로 둘 수 있지만, 실제 앱에서는 제거하는 것이 좋습니다.

      // StoresViewModel의 searchStores() 함수가 List<Store>를 반환하도록 해야 합니다.
      List<Store>? storesData = await _storesViewModel.searchStores();

      print("_fetchStores result: $storesData");
      return storesData; // 여기에서 반환된 데이터는 List<Store>? 타입이 되어야 합니다.
    } catch (error) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Store>?>(
        ...
        builder: (context, snapshot) {
          ...
          return const SizedBox.shrink(); // 반환하지 않을 경우 에러 발생 가능
        },
      ),
    );
  }
